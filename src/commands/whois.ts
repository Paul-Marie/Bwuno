    ...
    if (response.statusCode === 200) {
        try {
            ...
            if (answer.statusCode === 200) {
                const data = formateData(answer, base_url, link);
...
const formateData = (answer: any, base_url: string, link: string) => {
    const data: any = {};
    ...
    try {
        ...
        data.guild_members = soup.find('span', 'ak-infos-guildmembers').nextElement.nextElement.nextElement._text.trim().replace(/\s(.*)/g, '');
    } catch { };
    try {
