...
export const prefix = async (message: Message, line: string[], config: any): Promise<Message> => {
    if (line.length !== 2)
        return message.channel.send(format(sentences[config.lang].ERROR_INSUFICIENT_ARGUMENT, `${config.prefix}prefix [new_prefix]`));
    ...
    if (!message.member.hasPermission(['ADMINISTRATOR', 'VIEW_AUDIT_LOG']))
        message.channel.send(sentences[config.lang].ERROR_INSUFICIENT_PERMISSION);
